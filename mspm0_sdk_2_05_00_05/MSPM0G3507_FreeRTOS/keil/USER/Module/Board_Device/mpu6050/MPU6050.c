...
{
    uint8_t b;
    if (mpu6050_port_read(addr, reg, 1U, &b) == 0) {
        uint8_t mask = (0xFF << (bitStart + 1)) | (0xFF >> ((8 - bitStart) + length - 1));
        data <<= (8 - length);
        data >>= (7 - bitStart);
        b &= mask;
        b |= data;
        return (mpu6050_port_write(addr, reg, 1U, &b) == 0) ? 0U : 1U;
    }
    return 1;
}
...
uint8_t IICwriteBit(uint8_t dev, uint8_t reg, uint8_t bitNum, uint8_t data){
    uint8_t b;
    if (mpu6050_port_read(dev, reg, 1U, &b) != 0) {
        return 1U;
    }
    b = (data != 0) ? (b | (1 << bitNum)) : (b & ~(1 << bitNum));
    return (mpu6050_port_write(dev, reg, 1U, &b) == 0) ? 0U : 1U;
}

uint8_t IICreadBytes(uint8_t dev, uint8_t reg, uint8_t length, uint8_t *data)
{
    return (mpu6050_port_read(dev, reg, length, data) == 0) ? 0U : 1U;
}

int i2cRead(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *buf)
{
    return mpu6050_port_read(addr, reg, len, buf);
}

unsigned char I2C_ReadOneByte(unsigned char I2C_Addr,unsigned char addr)
{
    uint8_t b=0;
    (void) mpu6050_port_read(I2C_Addr, addr, 1U, &b);
    return b;
}
...
uint8_t MPU6050_testConnection(void) {
    return (MPU6050_getDeviceID() == 0x68U) ? 1U : 0U;
}
...
int DMP_Init(void)
{
    ...
    if ((i2cRead(devAddr, MPU6050_RA_WHO_AM_I, 1U, &device_id) != 0) ||
        (device_id != 0x68U)) {
        return -1;
    }
    if (mpu_init() != 0) {
